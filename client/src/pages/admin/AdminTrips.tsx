...
import {
  Plane, Plus, MapPin, Calendar, Trash2, Edit, ArrowLeft,
  Loader2, Hash, ChevronRight, Users
} from "lucide-react";

const STATUS_OPTIONS = ["planning", "confirmed", "active", "completed", "cancelled"] as const;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g., Hawaii Honeymoon" className="mt-1.5 font-sans" />
        </div>
        <div>
          <Label className="font-sans text-sm font-medium">Destination *</Label>
          <Input value={form.destination} onChange={e => setForm(f => ({ ...f, destination: e.target.value }))} placeholder="e.g., Maui, Hawaii" className="mt-1.5 font-sans" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="font-sans text-sm font-medium">Start Date</Label>
          <Input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} className="mt-1.5 font-sans" />
        </div>
        <div>
          <Label className="font-sans text-sm font-medium">End Date</Label>
          <Input type="date" value={form.endDate} onChange={e => setForm(f => ({ ...f, endDate: e.target.value }))} className="mt-1.5 font-sans" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
        ...
        <div>
          ...
          <Input value={form.confirmationCode} onChange={e => setForm(f => ({ ...f, confirmationCode: e.target.value }))} placeholder="e.g., NCT-2026-001" className="mt-1.5 font-sans" />
        </div>
      </div>
      <div>
        <Label className="font-sans text-sm font-medium">Cover Image URL</Label>
        <Input value={form.coverImageUrl} onChange={e => setForm(f => ({ ...f, coverImageUrl: e.target.value }))} placeholder="https://..." className="mt-1.5 font-sans" />
      </div>
      <div>
        <Label className="font-sans text-sm font-medium">Notes</Label>
        <Textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Internal notes about this trip..." className="mt-1.5 font-sans" rows={3} />
      </div>
      <Button type="submit" className="w-full bg-primary text-primary-foreground font-sans" disabled={createTrip.isPending}>
    ...
    <AdminLayout title="Trips" subtitle="Manage all client trips">
      ...
      {!isLoading && (!trips || trips.length === 0) && (
        <div className="text-center py-16">
          ...
          <Button className="mt-2 bg-primary text-primary-foreground font-sans" onClick={() => setOpen(true)}>
            Create First Trip
          </Button>
        </div>
      )}
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <Badge className={`text-xs font-sans ${STATUS_COLORS[trip.status]}`}>
                    {trip.status}
                  </Badge>
...
export function AdminTripDetail() {
  ...
  const { data: trip, isLoading, refetch } = trpc.trips.getById.useQuery({ id: tripId });
  const { data: itinerary, refetch: refetchItinerary } = trpc.itinerary.list.useQuery({ tripId }, { enabled: !!tripId });
  const { data: bookings, refetch: refetchBookings } = trpc.bookings.list.useQuery({ tripId }, { enabled: !!tripId });

  const [itinForm, setItinForm] = useState({
    dayNumber: "1", time: "", title: "", description: "",
    location: "", category: "activity" as const, confirmationNumber: "", notes: ""
  });
  const [itinOpen, setItinOpen] = useState(false);

  const updateTrip = trpc.trips.update.useMutation({
    onSuccess: () => { toast.success("Trip updated"); refetch(); },
    onError: (e) => toast.error(e.message),
  });

  const deleteTrip = trpc.trips.delete.useMutation({
    onSuccess: () => { toast.success("Trip deleted"); window.history.back(); },
    onError: (e) => toast.error(e.message),
  });

  const createItinItem = trpc.itinerary.create.useMutation({
    onSuccess: () => { toast.success("Itinerary item added"); setItinOpen(false); refetchItinerary(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  if (isLoading) return <AdminLayout title="Trip"><div className="flex justify-center py-16"><Loader2 className="w-8 h-8 animate-spin text-secondary" /></div></AdminLayout>;
  if (!trip) return <AdminLayout title="Not Found"><p className="text-muted-foreground font-sans">Trip not found.</p></AdminLayout>;
    ...
    <AdminLayout title={trip.title} subtitle={trip.destination}>
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h2 className="text-2xl font-serif font-bold text-foreground">{trip.title}</h2>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    {STATUS_OPTIONS.map(s => (
                      <SelectItem key={s} value={s} className="font-sans text-xs capitalize">{s}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <Button
              ...
              onClick={() => { if (confirm("Delete this trip?")) deleteTrip.mutate({ id: trip.id }); }}
            >
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-serif font-semibold text-foreground">Itinerary</h3>
          <Dialog open={itinOpen} onOpenChange={setItinOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle className="font-serif">Add Itinerary Item</DialogTitle>
              </DialogHeader>
              <div className="space-y-3 pt-2">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label className="font-sans text-sm">Day Number</Label>
                    <Input type="number" min="1" value={itinForm.dayNumber} onChange={e => setItinForm(f => ({ ...f, dayNumber: e.target.value }))} className="mt-1.5 font-sans" />
                  </div>
                  <div>
                    <Label className="font-sans text-sm">Time</Label>
                    <Input value={itinForm.time} onChange={e => setItinForm(f => ({ ...f, time: e.target.value }))} placeholder="e.g., 9:00 AM" className="mt-1.5 font-sans" />
                  </div>
                </div>
                <div>
                  <Label className="font-sans text-sm">Title *</Label>
                  <Input value={itinForm.title} onChange={e => setItinForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g., Check-in at Resort" className="mt-1.5 font-sans" />
                </div>
                <div>
                  <Label className="font-sans text-sm">Category</Label>
                  <Select value={itinForm.category} onValueChange={v => setItinForm(f => ({ ...f, category: v as any }))}>
                    <SelectTrigger className="mt-1.5 font-sans">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {["flight", "hotel", "activity", "dining", "transport", "free_time", "other"].map(c => (
                        <SelectItem key={c} value={c} className="font-sans capitalize">{c.replace("_", " ")}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Input value={itinForm.location} onChange={e => setItinForm(f => ({ ...f, location: e.target.value }))} placeholder="e.g., Grand Wailea Resort" className="mt-1.5 font-sans" />
                </div>
                <div>
                  <Label className="font-sans text-sm">Description</Label>
                  <Textarea value={itinForm.description} onChange={e => setItinForm(f => ({ ...f, description: e.target.value }))} className="mt-1.5 font-sans" rows={2} />
                </div>
                <div>
                  <Label className="font-sans text-sm">Confirmation Number</Label>
                  <Input value={itinForm.confirmationNumber} onChange={e => setItinForm(f => ({ ...f, confirmationNumber: e.target.value }))} className="mt-1.5 font-sans" />
                </div>
                <Button
                  className="w-full bg-primary text-primary-foreground font-sans"
                  disabled={!itinForm.title || createItinItem.isPending}
                  onClick={() => createItinItem.mutate({
                    tripId,
                    ...
                    confirmationNumber: itinForm.confirmationNumber || undefined,
                    notes: itinForm.notes || undefined,
                  })}
                >
                  {createItinItem.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
                  Add to Itinerary
                </Button>
              </div>
            </DialogContent>
        ...
        {(!itinerary || itinerary.length === 0) && (
          <div className="text-center py-8 border-2 border-dashed border-border rounded-xl">
            <p className="text-muted-foreground font-sans text-sm">No itinerary items yet. Add the first one!</p>
          </div>
        )}

        {Object.entries(groupedItinerary).map(([day, dayItems]) => (
          <div key={day} className="mb-4">
            <h4 className="text-sm font-sans font-semibold text-muted-foreground uppercase tracking-wider mb-2">Day {day}</h4>
                ...
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl border border-border bg-card">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-muted-foreground font-sans text-xs">
                      {item.time && `${item.time} · `}{item.category} {item.location && `· ${item.location}`}
                    </p>
                  </div>
                  <button
