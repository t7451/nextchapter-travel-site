...
import { Bell, Send, Info, AlertTriangle, AlertOctagon, Loader2 } from "lucide-react";
...
export default function AdminAlerts() {
  ...
  const handleSend = () => {
    ...
    Promise.all(targets.map(client =>
      ...
      toast.success(`Alert sent to ${targets.length} client${targets.length > 1 ? "s" : ""}!`);
  ...
  const severityConfig = {
    ...
    urgent: { icon: AlertOctagon, color: "text-red-600", bg: "bg-red-50 border-red-200" },
  };
  ...
  return (
    <AdminLayout title="Send Alerts" subtitle="Notify your clients with important travel updates">
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Form */}
        <Card>
          <CardContent className="p-6">
            <h3 className="font-serif font-semibold text-foreground mb-5">Compose Alert</h3>
            <div className="space-y-4">
              <div>
                ...
                <Select value={form.userId} onValueChange={v => setForm(f => ({ ...f, userId: v }))}>
                  ...
                  <SelectContent>
                    <SelectItem value="all" className="font-sans">All Clients</SelectItem>
                    {clients?.map(c => (
              ...
              <div>
                <Label className="font-sans text-sm font-medium">Severity</Label>
                <Select value={form.severity} onValueChange={v => setForm(f => ({ ...f, severity: v as any }))}>
                  <SelectTrigger className="mt-1.5 font-sans">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="info" className="font-sans">ℹ️ Info</SelectItem>
                    <SelectItem value="warning" className="font-sans">⚠️ Warning</SelectItem>
                    <SelectItem value="urgent" className="font-sans">🚨 Urgent</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
        ...
        {/* Preview */}
        <div>
          <h3 className="font-serif font-semibold text-foreground mb-4">Preview</h3>
          {form.title || form.content ? (
            <div className={`p-5 rounded-xl border ${preview.bg} flex items-start gap-4`}>
              <PreviewIcon className={`w-5 h-5 flex-shrink-0 mt-0.5 ${preview.color}`} />
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge className={`text-xs font-sans ${
                    form.severity === "info" ? "bg-blue-100 text-blue-800" :
                    form.severity === "warning" ? "bg-amber-100 text-amber-800" :
          ...
          <div className="mt-6 p-5 rounded-xl bg-muted/50 border border-border">
            <h4 className="font-serif font-semibold text-foreground mb-3 text-sm">Alert Tips</h4>
            <ul className="space-y-2 text-sm font-sans text-muted-foreground">
              <li className="flex items-start gap-2">
                <span className="text-secondary mt-0.5">•</span>
                Use <strong>Info</strong> for general updates (flight times, check-in reminders)
              </li>
              <li className="flex items-start gap-2">
                <span className="text-amber-500 mt-0.5">•</span>
                Use <strong>Warning</strong> for weather advisories or minor changes
              </li>
              <li className="flex items-start gap-2">
                <span className="text-red-500 mt-0.5">•</span>
                Use <strong>Urgent</strong> for cancellations, emergencies, or critical changes
              </li>
            </ul>
          </div>
        </div>
